...
A aplicação segue boas práticas de desenvolvimento, com **arquitetura bem definida**, **separação de camadas**, uso de **DTOs**, regras de negócio centralizadas na **camada de serviço** e **testes unitários** focados nas regras principais.
...
A transição de status deve respeitar obrigatoriamente a seguinte ordem:
...
**Observação:**  
O status **CANCELADO** pode ser aplicado a qualquer momento, independentemente da etapa atual.

---

## Classificação de Risco

A classificação de risco do projeto é calculada dinamicamente, com base no orçamento e no prazo:
...
- Cada projeto deve ter **no mínimo 1 e no máximo 10 membros**  
- Um membro não pode participar de mais de **3 projetos ativos**  
  (status diferente de **ENCERRADO** ou **CANCELADO**)  
...
> Atenção: Utilize essas credenciais para acessar o sistema.

Os endpoints do **Swagger** estão liberados **sem necessidade de autenticação**.
...
Os testes unitários estão concentrados principalmente na **camada de serviço**, onde estão localizadas as regras de negócio.
...
A cobertura de testes é focada nas regras de negócio, atendendo ao requisito mínimo de **70%**.
